...
class UAVProtocol(IProtocol):
    ...
    def handle_timer(self, timer: str) -> None:
        if timer == Timer.HEARTBEAT.value:
            ...
            elif self._mutual_exclusion_plugin.critical_section_status == CriticalSectionStatus.RELEASED:
    ...
    def handle_packet(self, message: str) -> None:
        ...
        if default_message.sender.agent == Agent.SENSOR:
            ...
            if self._battery_plugin.is_critical:
                self._mutual_exclusion_plugin.evaluate_entry_score(self.lamport_clock, self._battery_plugin.battery)

                if (
                    self._mutual_exclusion_plugin.critical_section_status == CriticalSectionStatus.WANTED
                    and self._mutual_exclusion_plugin.compare_entry_score(message.entry_score, message.sender.id)
